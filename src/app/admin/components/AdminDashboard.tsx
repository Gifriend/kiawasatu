...
export default function AdminDashboard() {
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("berita-images").upload(filePath, file)

      if (uploadError) throw uploadError

      const { data } = supabase.storage.from("berita-images").getPublicUrl(filePath)
        ...
        {showForm && (
          <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-800">{editingId ? "Edit Berita" : "Tambah Berita Baru"}</h2>
        ...
        <div className="grid gap-6">
          {beritaList.length === 0 ? (
            <div className="bg-white rounded-lg shadow p-8 text-center">
              <p className="text-gray-500">Belum ada berita. Tambahkan berita baru untuk memulai.</p>
            </div>
          ) : (
            beritaList.map((berita) => (
              <div key={berita.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex-1 p-6 flex flex-col justify-between">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{berita.judul}</h3>
                      <p className="text-gray-600 mb-3 line-clamp-2">{berita.konten}</p>
